...
const COUNTRY_INFO = {
  ...
  nameEn: 'Taiwan',
  flag: '🇹🇼',
  carriers: 'Chunghwa Telecom / Taiwan Mobile',
  highlight: '夜市美食、阿里山必备上网神器',
  ctaCountry: 'TW',
  faqs: [
    { q: '台湾eSIM用哪个运营商网络？', a: '我们的台湾eSIM运行在中华电信（Chunghwa Telecom）和台湾大哥大（Taiwan Mobile）等主流网络，覆盖全台包括台北、台中、台南、高雄及花莲、垦丁等热门旅游地。' },
    { q: '台湾山区如阿里山有信号吗？', a: '阿里山主要景点区域有基本信号覆盖，但部分高山步道可能信号较弱。日月潭、太鲁阁等热门景点覆盖良好。' },
    { q: '台湾eSIM可以打电话吗？', a: '我们提供的台湾eSIM主要为数据套餐，如需通话可通过LINE、WhatsApp等应用进行网络电话。部分套餐含本地语音通话，购买时请注意套餐说明。' },
    { q: '台湾和香港可以用同一张eSIM吗？', a: '台湾和香港需要分别购买本地eSIM，或购买覆盖两地的亚洲多国套餐。如您同时游览，建议咨询客服推荐最经济的方案。' },
  ],
  related: [
...
export default function TaiwanEsimPage({ products, minPrice }: PageProps) {
  const { t } = useI18n()
  const info = COUNTRY_INFO
  const siteUrl = 'https://simryoko.com'
  const pageUrl = `${siteUrl}/${info.slug}`
  const title = `台湾eSIM套餐 | 落地即用，最低$${minPrice.toFixed(2)}起 | SimRyoko`
  const description = `去台湾旅行？SimRyoko提供台湾eSIM套餐，${info.carriers}网络，落地即用，无需换卡。最低$${minPrice.toFixed(2)}起，支持信用卡/USDT支付。`
          ...
          <div className="grid md:grid-cols-2 gap-8">
            <div>
              ...
              <ol className="space-y-3">
                {['购买后邮件收到二维码', '进入 设置 → 蜂窝网络 → 添加eSIM → 扫描二维码', '到达台湾后开启eSIM数据即可使用'].map((s, i) => (
            ...
            <div>
              ...
              <ol className="space-y-3">
                {['购买后邮件收到二维码', '进入 设置 → 网络 → SIM卡 → 添加eSIM → 扫描二维码', '到达台湾后在移动网络设置中切换至eSIM'].map((s, i) => (
                  <li key={i} className="flex gap-3">
                    <span className="w-7 h-7 rounded-full bg-orange-100 text-orange-600 font-bold flex items-center justify-center flex-shrink-0 text-sm">{i + 1}</span>
                    <span className="text-gray-600 text-sm">{s}</span>
                  </li>
...
export const getStaticProps: GetStaticProps = async () => {
  const { local, regional } = getProductsByCountry('TW')
