...
const COUNTRY_INFO = {
  ...
  highlight: '东南亚背包客圣地，物美价廉',
  ctaCountry: 'VN',
  faqs: [
    { q: '越南eSIM用哪个运营商？', a: '我们的越南eSIM运行在Viettel、Mobifone、Vinaphone等越南主流运营商网络上，覆盖河内、胡志明市、岘港、会安、芽庄、下龙湾等热门旅游目的地。' },
    { q: '越南各城市间旅行网络稳定吗？', a: '越南南北狭长，主要城市和旅游景点均有良好4G覆盖。从河内到胡志明市的火车沿途、各大海滩度假区均有信号。部分山区（萨帕等）信号稍弱。' },
    { q: '越南eSIM价格贵吗？', a: '越南eSIM是我们价格最实惠的套餐之一，7天基础套餐价格极具竞争力，非常适合背包客和预算旅行者。' },
    { q: '越南可以用微信、抖音等中国APP吗？', a: '越南网络没有屏蔽微信、抖音等中国APP，可以正常使用。同时也可以访问Google、Facebook、YouTube等国际服务。' },
  ],
  related: [
...
export default function VietnamEsimPage({ products, minPrice }: PageProps) {
  const { t } = useI18n()
  const info = COUNTRY_INFO
  const siteUrl = 'https://simryoko.com'
  const pageUrl = `${siteUrl}/${info.slug}`
  const title = `越南eSIM套餐 | 落地即用，最低$${minPrice.toFixed(2)}起 | SimRyoko`
  const description = `去越南旅行？SimRyoko提供越南eSIM套餐，${info.carriers}网络，覆盖河内胡志明市，落地即用，无需换卡。最低$${minPrice.toFixed(2)}起，支持信用卡/USDT支付。`
          ...
          <div className="grid md:grid-cols-2 gap-8">
            <div>
              ...
              <ol className="space-y-3">
                {['购买后邮件收到二维码', '进入 设置 → 蜂窝网络 → 添加eSIM → 扫描二维码', '到达越南后开启eSIM数据即可使用'].map((s, i) => (
            ...
            <div>
              ...
              <ol className="space-y-3">
                {['购买后邮件收到二维码', '进入 设置 → 网络 → SIM卡 → 添加eSIM → 扫描二维码', '到达越南后在移动网络设置中切换至eSIM'].map((s, i) => (
                  <li key={i} className="flex gap-3">
                    <span className="w-7 h-7 rounded-full bg-orange-100 text-orange-600 font-bold flex items-center justify-center flex-shrink-0 text-sm">{i + 1}</span>
                    <span className="text-gray-600 text-sm">{s}</span>
                  </li>
...
export const getStaticProps: GetStaticProps = async () => {
  const { local, regional } = getProductsByCountry('VN')
