...
const COUNTRY_INFO = {
  ...
  nameCn: '新加坡',
  flag: '🇸🇬',
  carriers: 'Singtel / StarHub',
  highlight: '亚洲商务出行首选',
  ctaCountry: 'SG',
  faqs: [
    { q: '新加坡eSIM用哪个运营商网络？', a: '我们的新加坡eSIM运行在Singtel、StarHub等主流网络上，覆盖全新加坡，包括樟宜机场、乌节路、圣陶沙等热门区域，信号稳定。' },
    { q: '新加坡转机需要eSIM吗？', a: '如果转机时间超过6小时，建议购买新加坡短期套餐，方便在机场上网、联系酒店或安排行程。' },
    { q: '新加坡商务出行有推荐套餐吗？', a: '商务出行建议选择5GB或以上套餐，确保视频会议、邮件等商务需求不受流量限制影响。' },
    { q: '流量用完了怎么办？', a: '流量用完后可在SimRyoko购买新套餐，重新安装二维码即可。新加坡网速极快，4K视频消耗流量较多请注意。' },
  ],
  related: [
          ...
          <div className="grid md:grid-cols-2 gap-8">
            <div>
              ...
              <ol className="space-y-3">
                {['购买后邮件收到二维码', '进入 设置 → 蜂窝网络 → 添加eSIM → 扫描二维码', '到达新加坡后开启eSIM数据即可使用'].map((s, i) => (
            ...
            <div>
              ...
              <ol className="space-y-3">
                {['购买后邮件收到二维码', '进入 设置 → 网络 → SIM卡 → 添加eSIM → 扫描二维码', '到达新加坡后在移动网络设置中切换至eSIM'].map((s, i) => (
                  <li key={i} className="flex gap-3">
                    <span className="w-7 h-7 rounded-full bg-orange-100 text-orange-600 font-bold flex items-center justify-center flex-shrink-0 text-sm">{i + 1}</span>
                    <span className="text-gray-600 text-sm">{s}</span>
                  </li>
...
export const getStaticProps: GetStaticProps = async () => {
  const { local, regional } = getProductsByCountry('SG')
