...
const COUNTRY_INFO = {
  ...
  ctaCountry: 'MY',
  faqs: [
    { q: '马来西亚eSIM覆盖哪些区域？', a: '我们的马来西亚eSIM运行在Maxis、Celcom、Digi等主流运营商网络，覆盖吉隆坡、槟城、马六甲、沙巴、砂拉越及主要旅游景点，包括兰卡威岛、热带雨林等。' },
    { q: '马来西亚西马和东马（沙巴砂拉越）都有信号吗？', a: '是的，西马和东马均有覆盖。吉隆坡、槟城等大城市4G信号稳定，沙巴和砂拉越城区覆盖良好，偏远热带雨林地区信号有限。' },
    { q: '马来西亚和新加坡可以用同一张eSIM吗？', a: '马来西亚和新加坡通常需要分别购买，但我们有覆盖两国的东南亚多国套餐。如您计划游览两国，这可能更经济实惠，请咨询客服。' },
    { q: '马来西亚网络速度快吗？', a: '马来西亚主要城市4G网络速度良好，平均下载速度在20-50Mbps，完全满足日常社交、导航、视频通话需求。部分区域已开始5G覆盖。' },
  ],
  related: [
...
export default function MalaysiaEsimPage({ products, minPrice }: PageProps) {
  const { t } = useI18n()
  const info = COUNTRY_INFO
  const siteUrl = 'https://simryoko.com'
  const pageUrl = `${siteUrl}/${info.slug}`
  const title = `马来西亚eSIM套餐 | 落地即用，最低$${minPrice.toFixed(2)}起 | SimRyoko`
  const description = `去马来西亚旅行？SimRyoko提供马来西亚eSIM套餐，${info.carriers}网络，覆盖吉隆坡槟城沙巴，落地即用，无需换卡。最低$${minPrice.toFixed(2)}起，支持信用卡/USDT支付。`
  ...
  return (
    <>
          ...
          <div className="grid md:grid-cols-2 gap-8">
            <div>
              ...
              <ol className="space-y-3">
                {['购买后邮件收到二维码', '进入 设置 → 蜂窝网络 → 添加eSIM → 扫描二维码', '到达马来西亚后开启eSIM数据即可使用'].map((s, i) => (
            ...
            <div>
              ...
              <ol className="space-y-3">
                {['购买后邮件收到二维码', '进入 设置 → 网络 → SIM卡 → 添加eSIM → 扫描二维码', '到达马来西亚后在移动网络设置中切换至eSIM'].map((s, i) => (
                  <li key={i} className="flex gap-3">
                    <span className="w-7 h-7 rounded-full bg-orange-100 text-orange-600 font-bold flex items-center justify-center flex-shrink-0 text-sm">{i + 1}</span>
                    <span className="text-gray-600 text-sm">{s}</span>
                  </li>
      ...
      <section className="py-12 px-4 bg-white">
        <div className="max-w-4xl mx-auto">
          <h2 className="text-2xl font-bold text-gray-900 mb-6 text-center">东南亚其他热门目的地</h2>
...
export const getStaticProps: GetStaticProps = async () => {
  const { local, regional } = getProductsByCountry('MY')
  const all = [...local, ...regional]
  const sorted = all.sort((a, b) => a.price - b.price)
  const minPrice = sorted.length > 0 ? sorted[0].price : 3.00
  return {
    props: {
