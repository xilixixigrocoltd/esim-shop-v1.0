...
const COUNTRY_INFO = {
  ...
  nameEn: 'Hong Kong',
  flag: '🇭🇰',
  carriers: 'SmarTone / 3HK / CMHK',
  highlight: '购物美食天堂，转机必备',
  ctaCountry: 'HK',
  faqs: [
    { q: '香港eSIM覆盖哪些区域？', a: '香港eSIM使用SmarTone、3HK、中国移动香港等本地运营商，覆盖全港18区，包括九龙、港岛、新界及离岛，地铁、商场、酒店均有良好信号。' },
    { q: '香港eSIM可以打电话吗？', a: '部分套餐包含本地通话功能，纯数据套餐可通过WhatsApp、WeChat、FaceTime等应用进行语音或视频通话，效果稳定。' },
    { q: '用了香港eSIM可以访问大陆APP吗？', a: '香港网络不受内地防火长城限制，可自由访问Google、YouTube、Instagram等境外服务。如需访问大陆专属内容，可能需要单独设置。' },
    { q: '转机香港只待几小时，需要购买eSIM吗？', a: '如只在机场转机，机场提供免费WiFi，无需购买。如需出机场购物游览，建议购买香港eSIM，我们有1天短期套餐可供选择。' },
  ],
  related: [
...
export default function HongKongEsimPage({ products, minPrice }: PageProps) {
  const { t } = useI18n()
  const info = COUNTRY_INFO
  const siteUrl = 'https://simryoko.com'
  const pageUrl = `${siteUrl}/${info.slug}`
  const title = `香港eSIM套餐 | 落地即用，最低$${minPrice.toFixed(2)}起 | SimRyoko`
  const description = `去香港旅行购物？SimRyoko提供香港eSIM套餐，${info.carriers}网络，落地即用，无需换卡。最低$${minPrice.toFixed(2)}起，支持信用卡/USDT支付。`
          ...
          <div className="grid md:grid-cols-2 gap-8">
            <div>
              ...
              <ol className="space-y-3">
                {['购买后邮件收到二维码', '进入 设置 → 蜂窝网络 → 添加eSIM → 扫描二维码', '到达香港后开启eSIM数据即可使用'].map((s, i) => (
            ...
            <div>
              ...
              <ol className="space-y-3">
                {['购买后邮件收到二维码', '进入 设置 → 网络 → SIM卡 → 添加eSIM → 扫描二维码', '到达香港后在移动网络设置中切换至eSIM'].map((s, i) => (
                  <li key={i} className="flex gap-3">
                    <span className="w-7 h-7 rounded-full bg-orange-100 text-orange-600 font-bold flex items-center justify-center flex-shrink-0 text-sm">{i + 1}</span>
                    <span className="text-gray-600 text-sm">{s}</span>
                  </li>
...
export const getStaticProps: GetStaticProps = async () => {
  const { local, regional } = getProductsByCountry('HK')
