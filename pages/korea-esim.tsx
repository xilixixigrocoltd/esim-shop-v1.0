...
const COUNTRY_INFO = {
  ...
  ctaCountry: 'KR',
  faqs: [
    { q: '韩国用哪个运营商网络？', a: '我们的韩国eSIM运行在SK Telecom、KT、LG U+等主流网络上，覆盖首尔、釜山、济州岛等热门旅游城市及全国大部分地区。' },
    { q: '国行手机（双卡单待）能用吗？', a: '部分国行手机不支持eSIM功能，建议购买前确认您的手机支持eSIM。iPhone XS及以后机型、三星Galaxy S20及以后机型均支持eSIM。' },
    { q: '什么时候激活eSIM最好？', a: '建议到达韩国后再开启eSIM数据连接。购买后您会收到二维码，提前扫码安装到手机，到达韩国后在设置中开启即可使用。' },
...
export default function KoreaEsimPage({ products, minPrice }: Props) {
  const { t } = useI18n()
          ...
          <div className="grid md:grid-cols-2 gap-8">
            <div>
              ...
              <ol className="space-y-3">
                {['购买后邮件收到二维码', '进入 设置 → 蜂窝网络 → 添加eSIM → 扫描二维码', '到达韩国后开启eSIM数据即可使用'].map((s, i) => (
            ...
            <div>
              ...
              <ol className="space-y-3">
                {['购买后邮件收到二维码', '进入 设置 → 网络 → SIM卡 → 添加eSIM → 扫描二维码', '到达韩国后在移动网络设置中切换至eSIM'].map((s, i) => (
                  <li key={i} className="flex gap-3">
                    <span className="w-7 h-7 rounded-full bg-orange-100 text-orange-600 font-bold flex items-center justify-center flex-shrink-0 text-sm">{i + 1}</span>
                    <span className="text-gray-600 text-sm">{s}</span>
                  </li>
...
export const getStaticProps: GetStaticProps = async () => {
  const { local, regional } = getProductsByCountry('KR')
