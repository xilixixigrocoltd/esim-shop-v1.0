...
const COUNTRY_INFO = {
  ...
  ctaCountry: 'AU',
  faqs: [
    { q: '澳大利亚eSIM用哪个运营商？', a: '我们的澳大利亚eSIM运行在Telstra和Optus网络上，覆盖全澳主要城市及旅游景点，包括悉尼、墨尔本、布里斯班、珀斯、阿德莱德等。' },
    { q: '澳大利亚国土面积大，偏远地区有信号吗？', a: '主要城市和热门旅游景区（如大堡礁、乌鲁鲁）均有良好覆盖。偏远内陆地区信号有限，建议查询具体目的地覆盖情况。' },
    { q: '澳大利亚eSIM有长期套餐吗？', a: '是的，我们提供7天至30天套餐，非常适合留学生和长期旅行者。如需更长时间，可到期后重新购买。' },
    { q: '澳大利亚和新西兰能用同一张eSIM吗？', a: '澳大利亚和新西兰需要分别购买，或购买覆盖大洋洲的多国套餐。如您计划同时游览两国，可咨询客服推荐合适套餐。' },
  ],
  related: [
...
export default function AustraliaEsimPage({ products, minPrice }: PageProps) {
  const { t } = useI18n()
  const info = COUNTRY_INFO
  const siteUrl = 'https://simryoko.com'
  const pageUrl = `${siteUrl}/${info.slug}`
  const title = `澳大利亚eSIM套餐 | 留学旅行首选，最低$${minPrice.toFixed(2)}起 | SimRyoko`
  const description = `去澳大利亚旅行留学？SimRyoko提供澳大利亚eSIM套餐，${info.carriers}网络，落地即用，无需换卡。最低$${minPrice.toFixed(2)}起，支持信用卡/USDT支付。`
          ...
          <div className="grid md:grid-cols-2 gap-8">
            <div>
              ...
              <ol className="space-y-3">
                {['购买后邮件收到二维码', '进入 设置 → 蜂窝网络 → 添加eSIM → 扫描二维码', '到达澳大利亚后开启eSIM数据即可使用'].map((s, i) => (
            ...
            <div>
              ...
              <ol className="space-y-3">
                {['购买后邮件收到二维码', '进入 设置 → 网络 → SIM卡 → 添加eSIM → 扫描二维码', '到达澳大利亚后在移动网络设置中切换至eSIM'].map((s, i) => (
                  <li key={i} className="flex gap-3">
                    <span className="w-7 h-7 rounded-full bg-orange-100 text-orange-600 font-bold flex items-center justify-center flex-shrink-0 text-sm">{i + 1}</span>
                    <span className="text-gray-600 text-sm">{s}</span>
                  </li>
...
export const getStaticProps: GetStaticProps = async () => {
  const { local, regional } = getProductsByCountry('AU')
